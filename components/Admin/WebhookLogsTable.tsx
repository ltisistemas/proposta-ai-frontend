  ...
  const getAcaoBadge = (acao: string | null) => {
    switch (acao) {
      case "DOWNGRADE_TO_FREE":
        return (
          <span className="inline-flex items-center gap-1 text-[10px] font-bold bg-rose-50 text-rose-700 border border-rose-200/80 px-2 py-0.5 rounded-md">
            🔻 Downgrade Free
          </span>
        );
      case "PLAN_ACTIVATED":
        return (
          <span className="inline-flex items-center gap-1 text-[10px] font-bold bg-emerald-50 text-emerald-700 border border-emerald-200/80 px-2 py-0.5 rounded-md">
            ✨ Ativação PRO
          </span>
        );
